...
hub = pd.read_csv("/home/bharadwaj/work/Meeting_10/Meeting docs/Chris Feldman Docs/E3_trial1/E3_streamlines/streamlines_generator/hub_export.dat",na_values = None,skipinitialspace=True,skiprows=[0],sep="\s+",keep_default_na=False,names = ["X","Y","Z"])
...
LE_TE = pd.read_csv("/home/bharadwaj/work/Meeting_10/Meeting docs/Chris Feldman Docs/E3_trial1/E3_streamlines/streamlines_generator/unique_LE_TE_coordinates.dat",na_values = None,skipinitialspace=True,skiprows=[0],sep="\s+",keep_default_na=False,names = ["xLE","rLE","xTE","rTE"])
...
LE_TE.astype(float)
LE_TE = LE_TE.sort_values(by=['rLE'], ascending=False)
#print(LE_TE.tail())
...
# Remember to update the indices with the maximum and minimum value
dist_LE0_TE0 = np.sqrt( ((LE_TE.iloc[-1,0]-LE_TE.iloc[-1,2])**2)+((LE_TE.iloc[-1,1]-LE_TE.iloc[-1,3])**2) )
#print(dist_LE0_TE0)
norm_hub = hub.copy()
...
xhub_TE_diff = LE_TE.iloc[-1,2] - translated_hub.iloc[127,0]
yhub_TE_diff = LE_TE.iloc[-1,3] - translated_hub.iloc[127,1]
#print(xhub_TE_diff)
#print(yhub_TE_diff)
translated_hub['X'] = translated_hub['X'] + xhub_TE_diff
translated_hub['Y'] = translated_hub['Y'] + yhub_TE_diff - 30e-2
translated_hub = translated_hub.round(17)
...
#print(casing.head())
#   Part 1 of casing
casing.iloc[:95,1] = LE_TE.iloc[0,1]+30e-2
...
case3_points = intermediates([casing.iloc[93,0]+10e-1, LE_TE.iloc[0,1]+30e-2], [casing.iloc[125,0]-10e-2, LE_TE.iloc[0,3]+1e-2], nb_points=29)
case3_df = pd.DataFrame(case3_points, columns=['X','Y', 'Z'])
caseing1_df = casing1.append(case3_df, ignore_index=True)

#  Part 3 of casing
casing.iloc[124:,1] = LE_TE.iloc[0,3]+10e-2
...
def streamline_coordinates_construct(x_iloc):
   ...
   if x_iloc<110:
      ...
      for i in range(len(LE_df['X'])):
         y20 = translated_hub.iloc[x_iloc,1]+((LE_df.iloc[i,1])*((caseing1_dg.iloc[x_iloc,1]-translated_hub.iloc[x_iloc,1])/(LE_df.iloc[0,1]-LE_df.iloc[-1,1])))
      ...
      for i in range(len(TE_df['X'])):
         y20 = translated_hub.iloc[x_iloc,1]+((TE_df.iloc[i,1])*((caseing1_dg.iloc[x_iloc,1]-translated_hub.iloc[x_iloc,1])/(TE_df.iloc[0,1]-TE_df.iloc[-1,1])))
         temp_coords.append(y20)
         temp_coords = [round(num, 6) for num in temp_coords]
         sorted(temp_coords, reverse=True)
      stream_deav = max(temp_coords) - caseing1_dg.iloc[x_iloc, 1]
...
#Part1: Generate the points from LE and LE
...
#Part2: Apppend the generated points into a single dataframe
...
#Part3: Regroup the coodinates into separate dataframes containing individual streamline data.
strm_cutoff = int(len(TE_df))

streamline_01_coordiantes = streamlines_final_df.iloc[::strm_cutoff, :].reset_index(drop=True)
#print(streamline_01_coordiantes.tail())
streamline_02_coordiantes = streamlines_final_df.iloc[1::strm_cutoff, :].reset_index(drop=True)
streamline_03_coordiantes = streamlines_final_df.iloc[2::strm_cutoff, :].reset_index(drop=True)
streamline_04_coordiantes = streamlines_final_df.iloc[3::strm_cutoff, :].reset_index(drop=True)
streamline_05_coordiantes = streamlines_final_df.iloc[4::strm_cutoff, :].reset_index(drop=True)
streamline_06_coordiantes = streamlines_final_df.iloc[5::strm_cutoff, :].reset_index(drop=True)
streamline_07_coordiantes = streamlines_final_df.iloc[6::strm_cutoff, :].reset_index(drop=True)
streamline_08_coordiantes = streamlines_final_df.iloc[7::strm_cutoff, :].reset_index(drop=True)
streamline_09_coordiantes = streamlines_final_df.iloc[8::strm_cutoff, :].reset_index(drop=True)
streamline_10_coordiantes = streamlines_final_df.iloc[9::strm_cutoff, :].reset_index(drop=True)
streamline_11_coordiantes = streamlines_final_df.iloc[10::strm_cutoff, :].reset_index(drop=True)
streamline_12_coordiantes = streamlines_final_df.iloc[11::strm_cutoff, :].reset_index(drop=True)
streamline_13_coordiantes = streamlines_final_df.iloc[12::strm_cutoff, :].reset_index(drop=True)
streamline_14_coordiantes = streamlines_final_df.iloc[13::strm_cutoff, :].reset_index(drop=True)
streamline_15_coordiantes = streamlines_final_df.iloc[14::strm_cutoff, :].reset_index(drop=True)
streamline_16_coordiantes = streamlines_final_df.iloc[15::strm_cutoff, :].reset_index(drop=True)
streamline_17_coordiantes = streamlines_final_df.iloc[16::strm_cutoff, :].reset_index(drop=True)
streamline_18_coordiantes = streamlines_final_df.iloc[17::strm_cutoff, :].reset_index(drop=True)


#print(streamline_18_coordiantes.tail())

# Though this was a crude way to get the streamlines, a more smoother approach instead of calculating all the streamline coordninates using LE or TE coordintes would be to just calculate the first and last column of coodrdinates using the above method (Part1) and then linerarly interpolate any number of points between them such that you would have a smoother transition between inlet, LE, TE and outlet.


caseing1_dg = caseing1_dg.iloc[59:,:] #This chops off the casing extesnion and provides a more realistic view of the geometry.


# Part4: Create an output file such that you can paste the hub, streamlines and casing data into 3dbgb input file. The order is: Hub, Streamlines, casing.
list_0 = [0]
zero_df = pd.DataFrame(list(zip(list_0, list_0)), columns =['x_s', 'r_s'], dtype=int)


streamlines_hub_tblade3 = pd.DataFrame(list(zip(translated_hub['X'], translated_hub['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_df = streamlines_hub_tblade3.copy()
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_01_tblade3 = pd.DataFrame(list(zip(streamline_01_coordiantes['X'], streamline_01_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_01_tblade3 = streamlines_01_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_01_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_02_tblade3 = pd.DataFrame(list(zip(streamline_02_coordiantes['X'], streamline_02_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_02_tblade3 = streamlines_02_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_02_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_03_tblade3 = pd.DataFrame(list(zip(streamline_03_coordiantes['X'], streamline_03_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_03_tblade3 = streamlines_03_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_03_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_04_tblade3 = pd.DataFrame(list(zip(streamline_04_coordiantes['X'], streamline_04_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_04_tblade3 = streamlines_04_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_04_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_05_tblade3 = pd.DataFrame(list(zip(streamline_05_coordiantes['X'], streamline_05_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_05_tblade3 = streamlines_05_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_05_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_06_tblade3 = pd.DataFrame(list(zip(streamline_06_coordiantes['X'], streamline_06_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_06_tblade3 = streamlines_06_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_06_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_07_tblade3 = pd.DataFrame(list(zip(streamline_07_coordiantes['X'], streamline_07_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_07_tblade3 = streamlines_07_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_07_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_08_tblade3 = pd.DataFrame(list(zip(streamline_08_coordiantes['X'], streamline_08_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_08_tblade3 = streamlines_08_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_08_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_09_tblade3 = pd.DataFrame(list(zip(streamline_09_coordiantes['X'], streamline_09_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_09_tblade3 = streamlines_09_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_09_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_10_tblade3 = pd.DataFrame(list(zip(streamline_10_coordiantes['X'], streamline_10_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_10_tblade3 = streamlines_10_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_10_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_11_tblade3 = pd.DataFrame(list(zip(streamline_11_coordiantes['X'], streamline_11_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_11_tblade3 = streamlines_11_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_11_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_12_tblade3 = pd.DataFrame(list(zip(streamline_12_coordiantes['X'], streamline_12_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_12_tblade3 = streamlines_12_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_12_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_13_tblade3 = pd.DataFrame(list(zip(streamline_13_coordiantes['X'], streamline_13_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_13_tblade3 = streamlines_13_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_13_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_14_tblade3 = pd.DataFrame(list(zip(streamline_14_coordiantes['X'], streamline_14_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_14_tblade3 = streamlines_14_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_14_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_15_tblade3 = pd.DataFrame(list(zip(streamline_15_coordiantes['X'], streamline_15_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_15_tblade3 = streamlines_15_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_15_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_16_tblade3 = pd.DataFrame(list(zip(streamline_16_coordiantes['X'], streamline_16_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_16_tblade3 = streamlines_16_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_16_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_17_tblade3 = pd.DataFrame(list(zip(streamline_17_coordiantes['X'], streamline_17_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_17_tblade3 = streamlines_17_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_17_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_18_tblade3 = pd.DataFrame(list(zip(streamline_18_coordiantes['X'], streamline_18_coordiantes['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_18_tblade3 = streamlines_18_tblade3.round(7)
streamlines_df = streamlines_df.append(streamlines_18_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
streamlines_casing_tblade3 = pd.DataFrame(list(zip(caseing1_dg['X'], caseing1_dg['Y'])), columns =['x_s', 'r_s'], dtype=float)
streamlines_df = streamlines_df.append(streamlines_casing_tblade3, ignore_index = True)
streamlines_df = streamlines_df.append(zero_df, ignore_index = True)
print(streamlines_df)
streamlines_df.to_csv('tblade3_streamlines_w_format.dat', sep='\t', index=False)
